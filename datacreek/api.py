...
from datacreek.db import Dataset, SessionLocal, init_db
...
def api_list_datasets() -> list[str]:
...
def api_create_dataset(
    ...
    if NAME_PATTERN.match(payload.name) is None:
        raise HTTPException(status_code=400, detail="invalid dataset name")
    client = get_redis_client()
    if client is None:
        raise HTTPException(status_code=503, detail="Redis unavailable")
    key = f"dataset:{payload.name}"
    if client.exists(key):
        raise HTTPException(status_code=409, detail="Dataset already exists")
    driver = get_neo4j_driver()
    ds = DatasetBuilder(payload.dataset_type, name=payload.name)
    ds.redis_client = client
    ds.neo4j_driver = driver
    ds.history.append("Dataset created via API")
    ds._persist()
    if driver:
        driver.close()
    client.sadd("datasets", payload.name)
    return {"name": payload.name, "stage": ds.stage}
...
def api_dataset_detail(
    ...
    num_docs = sum(1 for _, data in nodes.nodes(data=True) if data.get("type") == "document")
    num_chunks = sum(1 for _, data in nodes.nodes(data=True) if data.get("type") == "chunk")
    return {
        "name": ds.name,
        "stage": ds.stage,
...
def list_user_datasets() -> list[str]:
...
def list_user_datasets_details() -> list[dict]:
...
def add_dataset(
    ...
    ds = create_dataset(
        db, None, payload.source_id, payload.path
    )  # pragma: no cover
...
def delete_persisted_dataset(
    ...
    # pragma: no cover
    _load_dataset(name)  # pragma: no cover
    # pragma: no cover
...
def dataset_history(
    ...
    ds = _load_dataset(name)  # pragma: no cover
...
def dataset_generate_route(
    ...
    celery_task = dataset_generate_task.apply_async(  # pragma: no cover
        args=[name, params or {}]  # pragma: no cover
    )  # pragma: no cover
    return {"task_id": celery_task.id}  # pragma: no cover
...
async def generate_async(
    ...
    celery_task = generate_task.apply_async(
        args=[None, params.src_id, params.content_type, params.num_pairs],
        kwargs={
...
async def curate_async(
    ...
    celery_task = curate_task.apply_async(
        args=[None, params.ds_id, params.threshold]
    )
    return {"task_id": celery_task.id}
...
async def save_async(
    ...
    celery_task = save_task.apply_async(
        args=[None, params.ds_id, params.fmt.value]
    )
    return {"task_id": celery_task.id}
